        ...
        public string ExecuteCommand(JsonRPCRequest request)
        {
            try
            {
                ...
                catch (CommandExecutionException ex)
                {
                    ...
                    return CreateErrorResponse(request.Id, ex.ErrorCode, ex.Message, ex.ErrorData);
                }
                catch (Exception ex)
                {
                    _logger.Error("Exception while executing command {0}: {1}", request.Method, ex.Message);
                    return CreateErrorResponse(request.Id, JsonRPCErrorCodes.InternalError, ex.Message);
                }
            }
            catch (Exception ex)
            {
                _logger.Error("Exception during command processing: {0}", ex.Message);
                return CreateErrorResponse(request.Id, JsonRPCErrorCodes.InternalError, $"Internal error: {ex.Message}");
            }
        }
