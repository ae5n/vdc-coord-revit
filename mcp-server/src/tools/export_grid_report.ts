...
export function registerExportGridReportTool(server: McpServer) {
  server.tool(
    "export_grid_report",
    ...
    async (args) => {
      try {
        const response = await withRevitConnection(async (revitClient) => {
          return await revitClient.sendCommand("export_grid_report", args);
        });
        return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
      } catch (error) {
        return { content: [{ type: "text", text: `export_grid_report failed: ${error instanceof Error ? error.message : String(error)}` }] };
      }
    }
  );
}
