...
export function registerRevitSuiteLevelReportTool(server: McpServer) {
  server.tool(
    "run_level_report",
    ...
    async (args) => {
      try {
        const response = await withRevitConnection(async (revitClient) => {
          return await revitClient.sendCommand("run_level_report", args);
        });
        return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
      } catch (error) {
        return { content: [{ type: "text", text: `run_level_report failed: ${error instanceof Error ? error.message : String(error)}` }] };
      }
    }
  );
}
